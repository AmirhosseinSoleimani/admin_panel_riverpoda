...
class PutContentRepository with GeneralRepository {
  ...
  PutContentRepository({required this.putContentRepository});

  Future<ApiResult> updateContent({required String title, required String description, required String id}) async {
    try {
      final response = await putContentRepository.updateContent(title: title, description: description, id: id);
